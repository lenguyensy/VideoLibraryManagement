...
public class UserModel {
	...
	private List<User> authenticateUserMDB(String email, String password) {
		DBCollection users = mongoDB.getCollection("users");
		BasicDBObject query = new BasicDBObject("Email", email).append("HashedPassword", MD5(password));
	...
	private String addUserMDB(User u) {
		...
		BasicDBObject doc = new BasicDBObject("MembershipNo", u.getMembershipNo()).
							...
							append("Total", u.getTotal()).
							append("City", u.getCity()).
							append("Zip", u.getZipCode()).
	...
	private List<User> getUsersListFromCursor(DBCursor cursor, int pageSize) {
		...
		while ( cursor.hasNext() && count < pageSize) {
			...
			u.setMonthlySubscriptionFee(Double.valueOf(String.valueOf(user.get("MonthlySubscriptionFee"))));
			u.setTotal(Double.valueOf(String.valueOf(user.get("Total"))));
			u.setTotalOutstandingMovies((Integer)user.get("TotalOutstandingMovies"));
